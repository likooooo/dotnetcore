...
public static unsafe class HelpMemoryHelper
{
...
class Test
{
    static unsafe void Main()
    {
        ...
        {
            const int Size = 256;
            buffer = (byte*)HelpMemoryHelper.Alloc(Size);
            for (int i = 0; i < Size; i++) buffer[i] = (byte)i;
            byte[] array = new byte[Size];
            fixed (byte* p = array) HelpMemoryHelper.Copy(buffer, p, Size);
            for (int i = 0; i < Size; i++) Console.WriteLine(array[i]);
            HelpMemoryHelper.ReAlloc(buffer,257);
            for (int i = 0; i < 257; i++) Console.WriteLine(buffer[i]);
        }
        finally
        {
            if (buffer != null) HelpMemoryHelper.Free(buffer);
        }
    }
}